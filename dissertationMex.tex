...
\shtsetup{
  ...
  secret-level = {保密等级},
  title = {无源物联网探索},
  title* = {Battery-free Internet of Things Exploration},
  ...
  supervisor = {梁俊睿~副教授},
  supervisor* = {Professor~Liang~Junrui},
  supervisor-institution = {上海科技大学信息科学与技术学院},
  discipline-level-1 = {信息与通信工程},
...
\usepackage{listings}
  % % this three is added by Max xxxx (i will use xxxx later on as a mark by max)
...
\begin{abstract}[flattitle]
  ...
  这里是本文的摘要，企在概括本文的研究目的、方法与结果。

  无联网\gls{iot}在我们的生活中十分重要!

\end{abstract}

\begin{abstract*}[flattitle]
 
  This is the abstract of this paper. I would like to introduce the main research object, the method used, and the

  So, the \gls{iot} is extremely important in our procution.
...
\begin{nomenclatures}
\header[单位]{符号}{说明}
\item[$\symup{\Omega}$]{$R_{cl}$}{Current-limiting resistor}
\item[$\symup{\Omega}$]{$R_{sen}$}{Sensing-circuit resistor}
\item[$\symup{\mathrm{F}}$]{$C_{sen}$}{Sensing-circuit capacitor}
\item[$\symup{\mathrm{F}}$]{$C_{sto}$}{Energy-storage capacitor}
\item[$\symup{\mathrm{V}}$]{$V_{th}$}{Classification threshold voltage}
\item[$\symup{\mathrm{V}}$]{$V_{adc}$}{Voltage sampled by the ADC}
\item[$\symup{\mathrm{V}}$]{$V_{oc}$}{Open-circuit output voltage of the QST}
\item[$\symup{\mathrm{s}}$]{$\tau$}{Time constant of the RC circuit, $\tau = R\cdot C$}
\end{nomenclatures}


% ==== 缩写 (按主题分类) ====
% 使用示例格式：\begin{nomenclatures}[缩写] \header{缩写}{全称} ... \end{nomenclatures}
\begin{nomenclatures}[缩写]
\header{缩写}{全称}
% --- Core concepts ---
...
\item{QNN}{Quantized Neural Network}
\item{tinyML}{Tiny Machine Learning}
\item{TFlite}{TensorFlow Lite}


% --- Energy / Power ---
\item{EH}{Energy Harvesting}
\item{EM}{Energy Management}
\item{ESM}{Energy Storage Module}
\item{EDC}{Energy-Driven Computing}
\item{PMIC}{Power Management Integrated Circuit}
\item{PV}{Photovoltaics}


% --- Security / Reliability ---
\item{RA}{Remote Attestation}
...
% --- Power/scheduling controls ---
\item{DPM}{Dynamic Power Management}
\item{DVFS}{Dynamic Voltage and Frequency Scaling}
\item{RM}{Resource Management}
...
\section{This section is test section of show latex technics}

\begin{figure}[htb]
  \centering
  \IfFileExists{fig/gaosq.jpg}{
    \includegraphics[width=0.15\columnwidth]{fig/gaosq.jpg}%
  }{
    \fbox{%
      \begin{minipage}[b][2.5cm][c]{0.75\columnwidth}%
        \centering\zihao{-5}\bfseries\sffamily\color{ShtRed}%
          missing gaosq.jpg in ./fig
      \end{minipage}%
    }%
  }%
  \bicaption{G圣Q}{This is the auther shengqi Gao}
  \label{img::auther_gaosq}
\end{figure}

You can quote this figure by ref~\ref{img::auther_gaosq}, and notice its width is 0.15.

\begin{align}
  P(a|B) &= \frac{P(a)P(B|a)}{P(B)} 
  \label{eq::bayesian_vER2}
\end{align}
同时重载了 \verb|\eqref|，使得公式编号格式修改后，其引用格式仍与 \textsf{amsmath} 无异：贝叶斯定理~\eqref{eq::bayesian_vER2}(notice, this is Bver2, we turn the a to lower case)。
(notice2: if you encounter ?? double questionmark at refer the equation, try to compile agian, idk why, but it sloves)

\subsubsection{You can use the subsubsection}
...
% insert the external ack file (relative path)
\input{sepTex/ack.tex}

\ifgraduate
\begin{resume}
  高圣淇，男，陕西省宝鸡市人，上海科技大学信息学院硕士研究生。

  通讯地址： xxx

  邮编： xxxx

  电子邮件： gaoshq2023@shanghaitech.edu.cn / douglasdmcuo98@gmail.com
\end{resume}
